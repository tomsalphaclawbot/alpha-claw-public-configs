...
| `TODO.md` | Legacy task queue (system of record is Beads in workspace) |
...
| Script | Purpose |
|--------|---------|
| `subagent-dispatcher.sh` | Queue top actionable Beads tasks for sub-agent kickoff |
...
**Queue execution is Beads-only (`bd`).**
...
- Every instruction from Tom → Beads task (`bd create`)
...
- Use Beads-native fields consistently (priority, labels, status, notes, dependencies)
...
- Finalize as done when complete (comment + close)
- Agents/sub-agents coordinate through Beads task updates (progress, handoffs, blockers, metadata)
- Sub-agent runs must include log-tracking references on the Beads task (session key, run id, log reference, heartbeat timestamp)
- Sub-agents may escalate/give up with a structured help-request comment when blocked repeatedly
