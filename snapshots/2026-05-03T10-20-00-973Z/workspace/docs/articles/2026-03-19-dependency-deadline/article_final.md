...
- **Behavioral stability.** Even when a model endpoint stays available, the model behind it can change. Fine-tuning updates, safety patches, capability adjustments — the model you tested against six months ago might not be the model you're calling today. Unlike a pinned package version, you can't pin a model's weights. There's no `model.lock` file. When the vendor updates the model behind your endpoint, your regression tests might still pass while the outputs shift in ways your downstream logic doesn't expect.
...
VPAR is not a small project. 860+ Python files is a substantial codebase. 25 prompt versions represent months of iteration. 17 research scans mean the team has been actively surveying the landscape. This is not a weekend hack that grew legs — it's a serious engineering effort with real investment. And yet: no dependency manifest for external model endpoints.

This isn't negligence. It's a category gap. When you think "dependency management," you think packages, libraries, frameworks — things with version numbers in a manifest file. Model endpoints don't pattern-match to that mental model, so they don't get tracked.

The GPT-4o retirement is a clean example because it's benign. The project wasn't shipping on GPT-4o. It was just evaluating it. If it had been the primary judge, and the retirement notice landed two weeks before launch, the scramble would have been real: re-run every evaluation on a new model, re-calibrate scoring thresholds against the new judge's tendencies, re-validate that ranking decisions made under the old judge still hold. That's not a one-day fix. That's weeks of re-work, and you'd be doing it under deadline pressure with a model you haven't had time to build intuition for.

The common response is: "We'll just swap models when it happens." This underestimates the cost. Swapping a judge model isn't like swapping a database driver. Every model scores differently. Rubric adherence patterns shift. Edge cases that one model handled cleanly become failure modes for another. The swap itself is the easy part — re-establishing confidence in all the downstream decisions is the expensive part.
