...
# top actionable bead by priority then oldest (open + unblocked in_progress)
candidate=""
...
unblocked_in_progress=[i for i in in_progress if 'blocked' not in labels(i)]
actionable=open_items + unblocked_in_progress
actionable.sort(key=lambda x:(x.get('priority',9),x.get('created_at',''),x.get('id','')))
if actionable:
    top=actionable[0]
    print(f"{top.get('id','')}\t{top.get('title','')}")
PY
)"
...
for row in recent:
    ...
    bead=row.get('beadId') or '<bead-id>'
    steer=row.get('steerMessage') or (
        f"Stall watchdog requested progress. Continue task execution now and post required Beads comment cadence on {bead}: "
        f"active/progress updates, structured blocker if blocked, or completion with validation evidence."
    )
    ...
    log={
        ...
        'beadId': row.get('beadId'),
        'reasons': row.get('reasons'),
        'requiredBeadsComment': row.get('requiredBeadsComment'),
        'commandRc': rc,
        'commandOutput': out,
    }
...
# Project-manager sweep + reconcile (best-effort): snapshot queue health and auto-fix hygiene drift.
if [[ -x "scripts/beads-pm-sweep.sh" ]]; then
  scripts/beads-pm-sweep.sh || true
fi
if [[ -x "scripts/beads-pm-reconcile.sh" ]]; then
  scripts/beads-pm-reconcile.sh || true
fi
