...
5. Read `LOW_TRUST_EXECUTION_CONTRACT.md` (execution boundary + trust model)
...
- **Context pressure rule (Tom directive, 2026-03-04):** whenever session context reaches **85% or higher**, immediately append a concise context snapshot to `memory/YYYY-MM-DD.md`.
- Snapshot minimum fields: `timestamp`, `session/chat`, `active tasks`, `recent decisions`, `next actions`, and any `blockers`.
- Save-on-85% is mandatory in every channel/session, not just private chats.
- **Immediate compression rule (Tom directive, 2026-03-05):** after writing the 85% snapshot, immediately run context compression in the same turn before continuing task work.
- **Post-compression reload rule (Tom directive, 2026-03-05):** immediately reload critical context from memory files (`memory/YYYY-MM-DD.md` current + previous day, relevant `memory/issues/*.md`, and `MEMORY.md` only in main session) before resuming.
- **Loop guard (Tom directive, 2026-03-05):** perform one compression pass per threshold crossing; do not re-trigger compression repeatedly unless context drops below threshold and later crosses it again.
- **Memory indexing rule (Tom directive, 2026-03-04):** whenever a new memory file is created (including `memory/YYYY-MM-DD.md` and `memory/issues/*.md`), run QMD indexing so it is searchable immediately.
- **Text > Brain** 📝
