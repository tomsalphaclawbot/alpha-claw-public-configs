...
2. **Traceability required**: actionable work is tracked in local Markdown task files (`tasks/`) by default.
...
- Local Markdown task files under `tasks/` are canonical.
- Every actionable request is tracked via the Markdown task workflow.
- Required intake metadata should be captured in task entries when relevant:
  - requester
  - source_channel
  - source_chat
  - source_message
- Beads remains available as an optional/legacy tool only when Tom explicitly requests it for a specific task/workflow.
...
- Findings must be captured in task/incident records with severity and mitigation (Markdown task files by default).
...
- Markdown task records (change task + rationale)
- commit history
- relevant policy docs
- (optional) Beads entry only when explicitly requested for that workflow
