...
run_step "02" "tasks_snapshot" "python3 scripts/tasks-snapshot.py"
run_step "04" "watchdog" "scripts/openclaw-watchdog.sh"
run_step "04b" "project_health_selfheal" "scripts/project-health-selfheal.sh"
run_step "05" "security_gate" "scripts/openclaw-security-gate.sh"
run_step "06" "heartbeat_slo_report" "scripts/heartbeat-slo-report.sh"
run_step "06b" "redteam_monthly_check" "scripts/redteam-monthly-check.sh"
run_step "06c" "financial_controls_report" "scripts/financial-controls-report.sh"
run_step "07a" "zoho_mail_healthcheck" "scripts/zoho-mail-healthcheck.sh"
run_step "07b" "zoho_mail_manage_inbox" "scripts/zoho-mail-manage-inbox.sh"
run_step "07c" "discord_openclaw_check" "scripts/discord-openclaw-check.sh"
...
run_step "11" "tasks_dispatcher" "scripts/tasks-dispatcher.sh"
...
# Temporarily disabled due to api usage limit issues; will re-enable once we have better monitoring and/or mitigation in place
# run_step "15d" "evergreen_autostart" "scripts/evergreen-autostart.sh"
# run_step "15e" "evergreen_singleton_check" "scripts/check-evergreen-singleton.sh"
run_step "16" "git_autocommit" "scripts/git-autocommit.sh"
run_step "18" "tasks_snapshot_final" "python3 scripts/tasks-snapshot.py"
...
if [[ -f "$RUN_DIR/18.log" ]]; then
  read -r OPEN_COUNT ACTIONABLE_COUNT < <(python3 - <<'PY' "$RUN_DIR/18.log"
import json,sys
p=sys.argv[1]
open_count='?'
actionable_count='?'
try:
    ...
    if isinstance(data,dict):
        open_count=str(data.get('open_count','?'))
        actionable_count=str(data.get('actionable_count','?'))
except Exception:
    pass
print(open_count, actionable_count)
PY
)
fi

summary_json="{\"runId\":\"$RUN_ID\",\"startedAt\":\"$run_start_iso\",\"endedAt\":\"$run_end_iso\",\"durationMs\":$run_duration_ms,\"status\":\"$overall_status\",\"runDir\":$(json_escape "${RUN_DIR#$ROOT_DIR/}"),\"openTasks\":$(json_escape "$OPEN_COUNT"),\"actionableTasks\":$(json_escape "$ACTIONABLE_COUNT"),\"steps\":$steps_json}"

echo "$summary_json" >> "$SUMMARY_FILE"
echo "$summary_json" > "$LATEST_FILE"

if ! grep -q "$RUN_ID" "$MEMORY_FILE" 2>/dev/null; then
  printf -- "- %s PT heartbeat run %s: status=%s, duration=%sms, open_tasks=%s, actionable_tasks=%s.\n" "$LOCAL_TIME" "$RUN_ID" "$overall_status" "$run_duration_ms" "$OPEN_COUNT" "$ACTIONABLE_COUNT" >> "$MEMORY_FILE"
fi
...
NEXT_ACTION="Advance highest-priority open task in tasks/ACTIVE.md (or move blocker with one concrete unblock step)."
...
if ! grep -q "$RUN_ID" "$IDLE_LOG_FILE" 2>/dev/null; then
  {
    ...
    printf -- "- %s PT run %s | status=%s | net_new_outputs=%s | blockers_changed=%s | outcome=%s | open_tasks=%s | actionable_tasks=%s | next=%s\n" "$LOCAL_TIME" "$RUN_ID" "$overall_status" "$NET_NEW_OUTPUTS" "$BLOCKERS_CHANGED" "$OUTCOME_CATEGORY" "$OPEN_COUNT" "$ACTIONABLE_COUNT" "$NEXT_ACTION"
  } >> "$IDLE_LOG_FILE"
fi
