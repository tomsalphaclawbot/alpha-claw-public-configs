...
description: Manage Outlook/Hotmail mailbox via Microsoft Graph OAuth2 (modern auth) for read, send, archive, delete, move, spam/junk checks, and folder operations. Use when working with [REDACTED_EMAIL] mail workflows, Cloudflare routing probes, inbox triage, or any Outlook automation that must avoid basic IMAP/POP/SMTP auth.
---
...
- `scripts/outlook-graph-token.py`
  - Shared expiry-aware token helper (refreshes 5 minutes before expiry, persists `obtained_at`).
- `scripts/outlook-graph-keepalive.sh`
  - Keepalive probe for cron; refreshes token if needed and validates mailbox API reachability.
...
# 4) Send email
skills/outlook-graph-mail/scripts/outlook-graph-send.sh "[REDACTED_EMAIL]" "Subject" "Body text"
