...
_openclaw_voicecall_call() {
  _arguments -C \
    "(--message -m)"{--message,-m}"[Message to speak when call connects]" \
    "(--to -t)"{--to,-t}"[Phone number to call (E.164 format, uses config toNumber if not set)]" \
    "--mode[Call mode: notify (hangup after message) or conversation (stay open)]"
}

_openclaw_voicecall_start() {
  _arguments -C \
    "--to[Phone number to call]" \
    "--message[Message to speak when call connects]" \
    "--mode[Call mode: notify (hangup after message) or conversation (stay open)]"
}

_openclaw_voicecall_continue() {
  _arguments -C \
    "--call-id[Call ID]" \
    "--message[Message to speak]"
}

_openclaw_voicecall_speak() {
  _arguments -C \
    "--call-id[Call ID]" \
    "--message[Message to speak]"
}

_openclaw_voicecall_end() {
  _arguments -C \
    "--call-id[Call ID]"
}

_openclaw_voicecall_status() {
  _arguments -C \
    "--call-id[Call ID]"
}

_openclaw_voicecall_tail() {
  _arguments -C \
    "--file[Path to calls.jsonl]" \
    "--since[Print last N lines first]" \
    "--poll[Poll interval in ms]"
}

_openclaw_voicecall_latency() {
  _arguments -C \
    "--file[Path to calls.jsonl]" \
    "--last[Analyze last N records]"
}

_openclaw_voicecall_expose() {
  _arguments -C \
    "--mode[off | serve (tailnet) | funnel (public)]" \
    "--path[Tailscale path to expose (recommend matching serve.path)]" \
    "--port[Local webhook port]" \
    "--serve-path[Local webhook path]"
}
...
_openclaw_voicecall() {
  ...
  _arguments -C \
     \
    "1: :_values 'command' 'call[Initiate an outbound voice call]' 'start[Alias for voicecall call]' 'continue[Speak a message and wait for a response]' 'speak[Speak a message without waiting for response]' 'end[Hang up an active call]' 'status[Show call status]' 'tail[Tail voice-call JSONL logs (prints new lines; useful during provider tests)]' 'latency[Summarize turn latency metrics from voice-call JSONL logs]' 'expose[Enable/disable Tailscale serve/funnel for the webhook]'" \
    "*::arg:->args"

  case $state in
    (args)
      case $line[1] in
        (call) _openclaw_voicecall_call ;;
        (start) _openclaw_voicecall_start ;;
        (continue) _openclaw_voicecall_continue ;;
        (speak) _openclaw_voicecall_speak ;;
        (end) _openclaw_voicecall_end ;;
        (status) _openclaw_voicecall_status ;;
        (tail) _openclaw_voicecall_tail ;;
        (latency) _openclaw_voicecall_latency ;;
        (expose) _openclaw_voicecall_expose ;;
      esac
      ;;
  esac
}
...
_openclaw_pairing_list() {
  _arguments -C \
    "--channel[Channel (discord, slack, telegram)]" \
