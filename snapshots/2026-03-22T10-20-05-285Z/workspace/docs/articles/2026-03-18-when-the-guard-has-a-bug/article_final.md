...
The guard has a bug. The guard always has a bug. The point is not to prevent the guard from ever being wrong — it's to make the failure survivable. Build diverse redundancy. Keep the plain-language logs. When the structured check and the memory disagree, stop. The real question is whether the rest of the system can catch it.
