...
1. **Daily cap check (required command):** run `python3 scripts/blog-publish-guard.py` from workspace root. If it reports `allowed=false`, do not autonomously publish another article unless Tom explicitly asks.
2. **Grounding check:** topic must come from recent memory/logs/learnings (last 1–3 days preferred), not free-floating philosophy.
3. **Evidence anchor (required):** capture at least 1 concrete source in `brief.md` (memory note, commit, incident, or shipped change) with explicit `Evidence anchor:` or `Source:` line.
4. **Coauthor requirement (hard):** post must include both Codex and Claude draft artifacts (`draft_codex*.md` and `draft_claude*.md`) plus `consensus.md` PASS.
5. **Dual-rating requirement (hard):** completed post must have both Codex and Claude `overall` scores in `projects/alpha-claw-web-site/content/article-ratings.json`.
...
- `python3 scripts/blog-quality-gate.py --article-id <slug-id> --article-dir <docs/articles/...>` returns `allowed=true`
