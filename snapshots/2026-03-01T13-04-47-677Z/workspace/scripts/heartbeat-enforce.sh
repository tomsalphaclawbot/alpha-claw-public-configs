...
# top open bead by priority then oldest (best-effort; tolerate bd/json failures)
candidate=""
if bd_json=$(bd list --status open --json 2>/dev/null); then
  candidate=$(printf '%s' "$bd_json" | python3 -c 'import sys,json
try:
 d=json.load(sys.stdin)
 d.sort(key=lambda x:(x.get("priority",9),x.get("created_at",""),x.get("id","")))
 print((d[0]["id"]+"\t"+d[0].get("title","")) if d else "")
except Exception:
 print("")')
fi
