...
- Assistant email (legacy/blocked): `[REDACTED_EMAIL]`
...
## Outlook Mail Access (routing destination)

- Destination mailbox: `[REDACTED_EMAIL]`
- IMAP: `outlook.office365.com:993` (SSL/TLS, OAuth2/Modern Auth)
- POP3: `outlook.office365.com:995` (SSL/TLS, OAuth2/Modern Auth)
- SMTP: `smtp-mail.outlook.com:587` (STARTTLS, OAuth2/Modern Auth)
- Credentials stored in Bitwarden item: `signup.live.com`
- Note: direct basic IMAP LOGIN from local scripts may fail under modern-auth-only policy; manual web verification may be required for Cloudflare destination-confirmation links.

## Google Access (gog CLI)

- `gog` is installed and authenticated for `[REDACTED_EMAIL]`.
- **Current incident (2026-02-24):** Google account/Gmail access is blocked/limited and under appeal. Treat Gmail automations as disabled/degraded.
- **Hard lock (critical):** treat Google/Gmail as disabled indefinitely. Do not attempt any Google/Gmail login/API actions until Tom explicitly re-enables them.
