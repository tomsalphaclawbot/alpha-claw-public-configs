...
This is the debugging tax of the polite lie: not just the time lost to the current incident, but the erosion of trust in every future 200 response.
...
If you're building on third-party APIs — especially orchestration layers that abstract multiple providers — silent fallback should be in your threat model. Some practical defenses:
...
**Treat cost and metadata as a control plane.** If the API won't tell you what it actually did, find the surface that does. Billing data, usage metrics, and call metadata are often more honest than API responses.
