...
run_step "18" "bd_queue_snapshot" "scripts/beads-queue-snapshot.py"
...
try:
    ...
    if isinstance(data,dict):
        ...
        # backward compatibility for old open-list payload
        open_count=str(len(data))
        actionable_count=str(len(data))
except Exception:
    pass
...
summary_json="{\"runId\":\"$RUN_ID\",\"startedAt\":\"$run_start_iso\",\"endedAt\":\"$run_end_iso\",\"durationMs\":$run_duration_ms,\"status\":\"$overall_status\",\"runDir\":$(json_escape "${RUN_DIR#$ROOT_DIR/}"),\"openBeads\":$(json_escape "$OPEN_COUNT"),\"actionableBeads\":$(json_escape "$ACTIONABLE_COUNT"),\"steps\":$steps_json}"

echo "$summary_json" >> "$SUMMARY_FILE"
echo "$summary_json" > "$LATEST_FILE"

if ! grep -q "$RUN_ID" "$MEMORY_FILE" 2>/dev/null; then
  printf -- "- %s PT heartbeat run %s: status=%s, duration=%sms, open_beads=%s, actionable_beads=%s.\n" "$LOCAL_TIME" "$RUN_ID" "$overall_status" "$run_duration_ms" "$OPEN_COUNT" "$ACTIONABLE_COUNT" >> "$MEMORY_FILE"
fi
...
- actionable_beads: $ACTIONABLE_COUNT
