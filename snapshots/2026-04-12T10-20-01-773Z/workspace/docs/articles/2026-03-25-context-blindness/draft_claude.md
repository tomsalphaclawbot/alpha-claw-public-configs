...
*Draft B: Claude role — conceptual depth, broader implications, operationalization*
...
|---|---|---|
| Baseline (no context) | 5 | $0.062 |
| Full context (3 fields pre-filled) | 7 | $0.085 |

The model with pre-filled context performed *worse*. More turns, higher cost, zero fields skipped. It asked for the caller's name on the first turn, exactly as if the context block didn't exist.
...
This is what makes context blindness a *class* of problem rather than a single failure. It's the failure mode for any architecture that treats the prompt as a flat document where position is irrelevant. The moment you assume presence equals processing, you've introduced a latent reliability bug that scales with prompt length.

## Position-First Design

The fix isn't louder prompts. Adding "IMPORTANT:" prefixes, bolding text, restating instructions — these are all strategies for increasing salience at a fixed position. They sometimes help at the margins, but they don't address the structural issue. You can't shout your way out of an attention trough.

What works is designing for position:

**Lead with state, follow with behavior.** The highest-attention zone is the first few hundred tokens. If you have pre-filled fields, they go before the behavioral instructions. The model should encounter "the caller's name is Sarah Mitchell" before it encounters "greet the caller and collect their information." Reverse the intuitive ordering — state first, personality second.

**Compress the active prompt.** A 2,000-character prompt with context at position 200 is more reliable than a 9,000-character prompt with context at position 7,000, even though the longer prompt has "more information." Factor reference material into retrieval. Keep the system prompt to what the model needs for the current turn, not everything it might ever need.

**Use structured state, not prose.** A block that reads `STATE: [x] name: Sarah Mitchell [ ] phone: UNKNOWN` creates a task-completion frame that prose descriptions don't. The checkbox pattern signals to the model that some work is done and some remains — a much stronger behavioral cue than a paragraph explaining what's known.

**Validate the first response.** If the model's first action after context injection asks for information you pre-filled, the injection failed. Build this check into your pipeline. Don't treat context injection as fire-and-forget — treat it as a hypothesis that needs verification on every call.
...
---
*Claude draft — 066 — 2026-03-25*
*Evidence: VPAR Task 9, 2026-03-25 — full_context condition: 7 turns / $0.085 vs baseline 5 turns / $0.062*
